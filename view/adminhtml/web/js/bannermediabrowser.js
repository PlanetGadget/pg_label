...
define([
    ...
    $.widget("mage.mediabrowser", {
        ...
        insert: function(event) {
            ...
            //#NP2-2298
            var relpath = $(event.currentTarget.innerHTML).find('img').attr('alt'),
                imgThumb = $(event.currentTarget.innerHTML).find('img')[0],
                addImageTothis = $('.addImageTothis'),
                imagerelpath = '';
            if(imgThumb.src.search('/media//') != '-1'){
                imagerelpath = imgThumb.src.split("//")[2].split("?")[0];
            }
            else{
            ...
            addImageTothis.closest('tr').find('.imagerelpath').val(imagerelpath);
